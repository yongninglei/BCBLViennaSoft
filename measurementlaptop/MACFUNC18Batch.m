function MACFUNC18Batch(SubjectType,varargin)
%%MACFUNC18Batch(SubjectType,varargin)
%Call with MACFUNC18Batch(SubjectType,'Fixation',Fixation,'SubjectName',SubjectName)
...
%Create input parser
p = inputParser;
p.addParameter('Fixation','cross',@isstr);
...
p.addParameter('MovingFixation',0,@isnumeric);
p.parse(varargin{:})
...
RetStimCell={'MeasurementlaptopFolderLocation',p.Results.MeasurementlaptopFolderLocation,'TR',2, ...
    'CustomStimSize',CustomStimSize,'PatientName',p.Results.SubjectName,'Fixation',Fixation, ...
    'ShiftStimX',p.Results.ShiftStimX,'ShiftStimY',p.Results.ShiftStimY,'Eyetracker',p.Results.Eyetracker, ...
    'CalibValidRatio',p.Results.CalibValidRatio};    
...
switch SubjectType
    ...
    case 'control'
        ...
        Input=input(['Next Stimulus is "Fullfield - Artificial Scotoma". ',InputStr],'s');
        if contains(Input,'quit'); return; end
        if ~contains(Input,'skip'); RetStim('StimType','fullfield','SimulatedScotoma',2,RetStimCell{:}); end
        
        Input=input(['Next Stimulus is "WedgeRings - Artificial Scotoma". ',InputStr],'s');
        if contains(Input,'quit'); return; end
        if ~contains(Input,'skip'); RetStim('StimType','wedgeringsaltnojump','SimulatedScotoma',2,RetStimCell{:}); end
        
        Input=input(['Next Stimulus is "Eightbars - Artificial Scotoma". ',InputStr],'s');
        if contains(Input,'quit'); return; end
        if ~contains(Input,'skip'); RetStim('StimType','eightbars_blanks','SimulatedScotoma',2,RetStimCell{:}); end
