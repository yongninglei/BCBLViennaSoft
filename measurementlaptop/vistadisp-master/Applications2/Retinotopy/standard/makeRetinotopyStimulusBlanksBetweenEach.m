...
if params.insertBlanks.do,
    ...
    if isempty(params.loadMatrix),
        ...
        %Index=(duration.cycle.seconds/duration.stimframe)*[2:2:params.ncycles];%used
        %for the 4 blanks between 2 and 3, 4 and 5, 6 and 7 and afer 8
        %OffPeriod=(duration.cycle.seconds/3)/duration.stimframe;
        OffPeriod=params.tr/duration.stimframe;%just one wedge long
        Index=[OffPeriod+1:2*OffPeriod:length(sequence)]
        ...
        for IndexLength=1:length(Index)
            ...
            sequence=[sequence(1:Index(IndexLength)-1);ones(OffPeriod,1)*add;sequence(Index(IndexLength)+OffPeriod:end)];
            %sequence=[sequence(1:Index(IndexLength));ones(OffPeriod,1)*add];
        end
