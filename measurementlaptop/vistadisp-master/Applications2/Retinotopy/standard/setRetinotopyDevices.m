...
elseif length(params.devices.keyInputExternal) > 1
    ...
    params.devices.keyInputExternal = params.devices.keyInputExternal(2);
end;
 
disp(['[',mfilename,'] Getting subjects responses from device ',num2str(params.devices.keyInputExternal)]) 
disp(['[',mfilename,'] Getting experimentor''s responses from device ',num2str(params.devices.keyInputInternal)]) 
