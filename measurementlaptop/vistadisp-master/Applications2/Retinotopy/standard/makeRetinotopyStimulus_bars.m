    ...
    for imgNum=1:halfNumImages
        ...
        switch params.experiment
            case {'8 bars','8 bars with blanks','8 bars (slow)', '8 bars with blanks, fixed check size', '8 bars with blanks thin', '8 bars with blanks thick'}
                ...
                %bk = 128
                %params.xfailure
                if isfield(params,'xfailure')&&isfield(params,'yfailure')&&isfield(params,'myradius')
                    
                    if isfield(params,'myradius')&&strcmp(params.mycircle,'full')
                        %for xlocation=1:768%1024
                        for xlocation=1:min(params.display.numPixels)
                        %    for ylocation=1:768%1024
                             for ylocation=1:min(params.display.numPixels)
                        
                                   
                                if ((xlocation-params.xfailure)^2+(ylocation-params.yfailure)^2)<params.myradius^2 
                                    
                                    images(ylocation,xlocation,(imgNum-1).*numMotSteps+1:imgNum.*numMotSteps) = uint8(128);%uint8(0); %128 = gray, 0 = black
                                    
                                end
                                
                                %if ((xlocation-params.xfailure2)^2+(ylocation-params.yfailure2)^2)<params.myradius^2 
                                %    
                                %    images(ylocation,xlocation,(imgNum-1).*numMotSteps+1:imgNum.*numMotSteps) = uint8(0);%uint8(128);
                                %    
                                %end
                                
                                %if ((xlocation-params.xfailure3)^2+(ylocation-params.yfailure3)^2)<params.myradius^2 
                                %    
                                %    images(ylocation,xlocation,(imgNum-1).*numMotSteps+1:imgNum.*numMotSteps) = uint8(0);%uint8(128);
                                %    
                                %end
                                
                                %if ((xlocation-params.xfailure4)^2+(ylocation-params.yfailure4)^2)<params.myradius^2 
                                %    
                                %    images(ylocation,xlocation,(imgNum-1).*numMotSteps+1:imgNum.*numMotSteps) = uint8(0);%uint8(128);
                                %    
                                %end
                    ...
                    if isfield(params,'myradius')&&strcmp(params.mycircle,'fullhalfleft')
                        for xlocation=1:768
                            for ylocation=1:768
                                
                                if xlocation>383
                                    
                                    images(ylocation,xlocation,(imgNum-1).*numMotSteps+1:imgNum.*numMotSteps) = uint8(128);
                                
                                elseif ((xlocation-params.xfailure)^2+(ylocation-params.yfailure)^2)<params.myradius^2&&xlocation<384
                    ...
                    if isfield(params,'myradius')&&strcmp(params.mycircle,'fullhalfright')
                        for xlocation=1:768
                            for ylocation=1:768
                    ...
                    if isfield(params,'myradius')&&strcmp(params.mycircle,'empty')
                        for xlocation=1:1024
                            for ylocation=1:1024
                                if ((xlocation-params.xfailure)^2+(ylocation-params.yfailure)^2)>params.myradius^2
                    ...
                    if isfield(params,'myradius')&&strcmp(params.mycircle,'emptyhalfleft')
                        for xlocation=1:768
                            for ylocation=1:768
                    ...
                    if isfield(params,'myradius')&&strcmp(params.mycircle,'emptyhalfright')
                        for xlocation=1:768
                            for ylocation=1:768
                ...
                elseif isfield(params,'xfailure')&&isfield(params,'yfailure')
                   
                    images(params.yfailure,params.xfailure,(imgNum-1).*numMotSteps+1:imgNum.*numMotSteps)=uint8(128); 
                    
                end
                
                 if isfield(params,'DiagMatrix')
        
                 %images(params.DiagMatrix==1,(imgNum-1).*numMotSteps+1:imgNum.*numMotSteps)=uint8(0);                

                    for p=(imgNum-1).*numMotSteps+1:imgNum.*numMotSteps
                        images(find(params.DiagMatrix~=0)+(m*n)*(p-1))=uint8(0);
                    end
...
sequence = ones(round(duration.cycle.stimframes./2./halfNumImages),1)*(1:params.temporal.motionSteps:params.temporal.motionSteps*halfNumImages);

%RANDOMIZATION

%randomizationvector=randperm(size(sequence,2));

%save randomizationvector randomizationvector

%load randomizationvector

%sequence = sequence(:,randomizationvector);
...
rev = [];
for nnn=1:4,
    rev = [rev; flipud(sequence(sep(nnn):sep(nnn+1)-1))]; %#ok<*AGROW>
end;
...
if params.insertBlanks.do
    ...
    BlankLength= 1/3; %Length of a Blank Period in relation to a Bar Period
    NBlanks=4; %Number of Blank Periods

    BlankImageNumber=size(images,3)+1;
    images(:,:,BlankImageNumber) = uint8(bk); %Define Blank Image
    
    if isfield(params,'xfailure')&&isfield(params,'yfailure')
        ...
        for xlocation=1:1024
            ...
            for ylocation=1:1024
                ...
                %Circles
                if isfield(params,'myradius')
                    
                    if ((xlocation-params.xfailure)^2+(ylocation-params.yfailure)^2)<params.myradius^2
                        
                        images(ylocation,xlocation,BlankImageNumber) = uint8(128);%uint8(0);
...
%                     end
                    ...
                    if sum(xlocation==params.xfailure) && sum(ylocation==params.yfailure)
    ...
    if isfield(params,'DiagMatrix')
            ...
            %images(params.DiagMatrix==1,BlankImageNumber) = uint8(0);%uint8(128); 
            images(find(params.DiagMatrix~=0)+(m*n)*(BlankImageNumber-1))=uint8(0);
            images(find(params.DiagMatrix~=0)+(m*n)*(p-1))=uint8(0);
                
    end
    
    BarPeriodsec=params.period/8;
    BarPeriod=BarPeriodsec*params.motionSteps*params.tempFreq;
    BarPeriod2=BarPeriod*2; % Blanks are only shown after 2 Bar Periods
    %if params.tr==1.25
    %    BlankPeriodsec=12.5 %length of blanks
    %else
        BlankPeriodsec=BarPeriodsec*BlankLength; %length of blanks
        BlankPeriodTR=BlankPeriodsec/params.tr;
        if BlankPeriodTR~=ceil(BlankPeriodTR)
            BlankPeriodsec=round(BlankPeriodTR)*params.tr;
           disp(['[',mfilename,'] Rounded the Blank Periods to ',num2str(BlankPeriodsec),' seconds to match with a TR of ',num2str(params.tr),' seconds'])            
        end
        
    %end
    BlankPeriod=BlankPeriodsec*params.motionSteps*params.tempFreq; %length of blanks in unit of displayed images

    TotalBlankPeriod=BlankPeriod*NBlanks;

    BlankSequenceSize=size(sequence,1)+TotalBlankPeriod;

    BlankSequence=zeros(BlankSequenceSize,1);


    BlankSequence(1:BarPeriod2)=sequence(1:BarPeriod2);
    BlankSequence(BarPeriod2+1:BarPeriod2+BlankPeriod)=BlankImageNumber;

    for s=1:NBlanks-1

    BlankSequence(s*(BarPeriod2+BlankPeriod)+1:s*(BarPeriod2+BlankPeriod)+BarPeriod2)=sequence(s*BarPeriod2+1:(s+1)*BarPeriod2);
    BlankSequence(s*(BarPeriod2+BlankPeriod)+BarPeriod2+1:s*(BarPeriod2+BlankPeriod)+BarPeriod2+BlankPeriod)=BlankImageNumber;

    end

    % sequence(90:180)=size(images,3);
    % if params.insertBlanks.do,
    ...
    % end;
    save BlankSequence BlankSequence
    clear sequence
...
%% If sequence is not exactly scan length we extend it a little bit
TotalStimLengthInScans=(length(stimulus.seq)/(params.motionSteps*params.tempFreq))/params.tr;
if ceil(TotalStimLengthInScans)~=TotalStimLengthInScans
    disp(['[',mfilename,'] Extending stimulus with blanks to match with a whole TR. (From ',num2str(TotalStimLengthInScans),' scans to ',num2str(ceil(TotalStimLengthInScans)),' scans.)'])
    NewTotalLengthInFrames=ceil(TotalStimLengthInScans)*(params.motionSteps*params.tempFreq)*params.tr;
    stimulus.seq(end+1:NewTotalLengthInFrames)=stimulus.seq(end);
    stimulus.fixSeq(end+1:NewTotalLengthInFrames)=stimulus.fixSeq(end);
    stimulus.seqtiming(end+1:NewTotalLengthInFrames)=stimulus.seqtiming(end)+([1:(NewTotalLengthInFrames-length(stimulus.seqtiming))]*(1/(params.motionSteps*params.tempFreq)));
end
