function stimulus = makeRetinotopyStimulusCustom(params)
...
if ~isempty(params.loadMatrix),
    ...
    countvarrel=0;
    countvarabs=0;
    countvarBlinkSegments=1;
    BlankNumber=1;
    %Struct which describes the sequence of blinking segments 
    ImageDuration=6; %In frames (Time/TR)
    
    
    BlinkingSegments=DefineBlinkingSegments(params.numImages,ImageDuration);
    ...
    for imgNum=1:params.numImages
        ...
        switch params.type;
            ...
            case 'stationary-bottom-wedge-on-off'
                if imgNum == 1,
            ...
            case 'stationary-top-wedge-on-off'
                if imgNum == 1,
                    loAngle = pi/2 - wedgeWidth/2;
                    hiAngle = pi/2 + wedgeWidth/2;
                elseif imgNum == 2
                    loAngle = 0;
                    hiAngle = 0;
                end
            ...
            case 'stationary-double-wedge-on-off'
                if imgNum == 1,
                    loAngle = 3*pi/2 - wedgeWidth/2;
                    hiAngle = 3*pi/2 + wedgeWidth/2;
                    loAngle2 = pi/2 - wedgeWidth/2;
                    hiAngle2 = pi/2 + wedgeWidth/2;
            ...
            case 'stationary-double-wedge-on-off-halfleft'
                if imgNum == 1,
                    loAngle = 3*pi/2 - wedgeWidth/2;
                    hiAngle = 3*pi/2;
                    loAngle2 = pi/2;
            ...
            case 'stationary-double-wedge-on-off-halfright'
                if imgNum == 1,
                    loAngle = 3*pi/2;
                    hiAngle = 3*pi/2 + wedgeWidth/2;
                    loAngle2 = pi/2 - wedgeWidth/2;
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%
        if strcmp(params.experiment,'multifocal'); 
            
            countvarrel=countvarrel+1;
            countvarabs=countvarabs+1;
            
            
            
            if countvarrel>ImageDuration || imgNum==1
                ...
                %BlinkingSegments=round(FromSeg+rand(1,NSegments)*(ToSeg-FromSeg));
                
                %Randomization Algorithm end
                

                %window=CreateMultifocalSegments(BlinkingSegments);
                
                BlinkingVec=BlinkingSegments(countvarBlinkSegments,:);
                
                BlinkingVec(BlinkingVec(1,:)==0)=[];
                

                window=CreateMultifocalSegments(BlinkingVec);
                
                
                
                countvarBlinkSegments=countvarBlinkSegments+1;
                countvarrel=1;
            ...
            %Inserting Blank Periods
            %if 36*BlankNumber+(BlankNumber-1)*ImageDuration < countvarabs
                
            %    window=window+ones(m,n)*bk;
            ...
            window = ( ((theta>=loAngle & theta<hiAngle) | (hiAngle>2*pi & theta<mod(hiAngle,2*pi))) & ((r>=loEcc & r<=hiEcc)) & r<outerRad);
        ...
        %for double wedge, double wedges
        
        if strcmp(params.type,'stationary-double-wedge-on-off') || strcmp(params.type,'stationary-double-wedge-on-off-halfleft') || strcmp(params.type,'stationary-double-wedge-on-off-halfright')
                ...
                for xlocation=1:1024
                    ...
                    for ylocation=1:1024
                        ...
                        if ((xlocation-params.xfailure)^2+(ylocation-params.yfailure)^2)<params.myradius^2
                            ...
                            images(ylocation,xlocation,(imgNum-1).*numMotSteps+1:imgNum.*numMotSteps) = uint8(128);%uint8(128);
