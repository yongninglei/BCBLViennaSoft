...
%     innerRad=outerRad/2;
    ...
    for imgNum=1:params.numImages
        ...
        %Insert Local Scotoma and Fixation Aids
...
if params.insertBlanks.do,
    ...
    if isempty(params.loadMatrix),
        %sequence(seq2==1) = add;
        %Add Blanks after each period
        Index=(duration.cycle.seconds/duration.stimframe)*[2:2:params.ncycles];
        
        if strcmp(params.experiment,'center-surround with blanks')
            OffPeriodsec=duration.cycle.seconds/2;
        else
            OffPeriodsec=duration.cycle.seconds/3;
        end
        OffPeriodTR=OffPeriodsec/params.tr;
        
        if OffPeriodTR~=ceil(OffPeriodTR)
            OffPeriodsec=round(OffPeriodTR)*params.tr;
            disp(['[',mfilename,'] Rounded the Blank Periods to ',num2str(OffPeriodsec),' seconds to match with a TR of ',num2str(params.tr),' seconds'])
        end
        
        OffPeriod=OffPeriodsec/duration.stimframe;
        
        for IndexLength=1:length(Index)
            sequence=[sequence(1:Index(IndexLength)+(IndexLength-1)*OffPeriod);ones(OffPeriod,1)*add;sequence((Index(IndexLength)+(IndexLength-1)*OffPeriod+1):end)];
        end
        ...
        if isfield(params,'DiagMatrix')
            ...
            images(find(params.DiagMatrix~=0)+(m*n)*(add-1))=uint8(0);
            images(find(params.DiagMatrix~=0)+(m*n)*(p-1))=uint8(0);
            
        end
        
        %Fixation Dot
        fixSeq=[fixSeq;fixSeq(1:length(sequence)-length(fixSeq))];
