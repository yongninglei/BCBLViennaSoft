...
params.display.fixColorRgb    = [0 200 0 255; 200 0 255 255]; %G/M
%params.display.fixColorRgb    = [0 255 0 255; 255 255 0 255]; %G/Y
...
%                                         255 255 255 255];
    ...
    case {'my thin cross'}
        ...
        params.display.fixX          = round(dim.x./2);
        params.display.fixY          = round(dim.y./2);
        params.display.fixSizePixelsFixationDisk = 6;
        params.display.fixColorRgbFixationDisk   = flipud(params.display.fixColorRgb); %G/M
        
        %Fixation cross
        if isfield(params,'FixationandBackgroundImageSize')
            dim.y=params.FixationandBackgroundImageSize;
            dim.x=params.FixationandBackgroundImageSize;
        elseif isfield(params,'ImageSize')
            dim.y=params.ImageSize;
            dim.x=params.ImageSize;
        else
            dim.y=params.StimSizePixel.y;
            dim.x=params.StimSizePixel.x;
        end
                
        params.display.fixSizePixels  = [2, 2];
        dim.ycoord = [1:dim.y dim.y:-1:1]+(params.display.numPixels(2)/2-dim.y/2); % assume ydim is smallest
        dim.xcoord = ([1:dim.y 1:dim.y] + round(-dim.y/2+dim.x/2))+((params.display.numPixels(1)/2-dim.x/2));
        params.display.fixCoords{1} = [dim.xcoord;dim.ycoord];
        params.display.fixCoords{2} = [dim.xcoord;dim.ycoord];
