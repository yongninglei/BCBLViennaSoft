...
% Use Lines instead of Ovals (we have to draw multiple lines as max. line
% size is 7)
if isfield(el,'pluscalibrationtarget')
    HorizontalLine=[0,width;calxy(2),calxy(2)];
    VerticalLine=[calxy(1),calxy(1);0,heigth];
    LineVector=[HorizontalLine,VerticalLine];
    
    LineShifts=[-6,-3,3,6];
    for i=1:length(LineShifts)
        ShiftedHorizontalLine=HorizontalLine;
        ShiftedVerticalLine=VerticalLine;
        
        ShiftedHorizontalLine(2,:)=HorizontalLine(2,:)+LineShifts(i);
        ShiftedVerticalLine(1,:)=VerticalLine(1,:)+LineShifts(i);
        
        LineVector=[LineVector,ShiftedHorizontalLine,ShiftedVerticalLine];
    end
    
    Screen('DrawLines',eyewin, LineVector, 7, el.calibrationtargetcolour);
